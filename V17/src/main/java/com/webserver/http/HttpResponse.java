...
public class HttpResponse {
    ...
    /*
        java.io.ByteArrayOutputStream是一个低级流，其内部维护一个字节数组，通过当前读写的
        数据实际上就是保存在内部的字节数组上了。
     */
    ...
    private void beforeFlush(){
        ...
        if(entity==null){//前提是没有以文件形式设置过正文
            writer.flush();//先确保通过PrintWriter写出的内容都写入ByteArrayOutputStream内容数组
...
//            }
    ...
    private void sendContent(){
        ...
        if(data.length>0){//若存在数据，则将它作为正文发送给客户端
            try {
